...
export function listInsights() {
  ...
  insights.sort((a, b) => {
    const dateA = String(a.publishDate || "");
    const dateB = String(b.publishDate || "");

    if (dateA !== dateB) {
      return dateB.localeCompare(dateA);
    }

    return String(b.__sourceFile || "").localeCompare(String(a.__sourceFile || ""));
  });

  return insights.map(({ __sourceFile, ...insight }) => insight);
}
